shinyServer(function(input, output, session) {
  ...
  ModuleESimulation = reactive({
    ...
    parameters$event_prior_distribution = ModuleEPriorDistribution(expected = log(2) / as.numeric(input$event_median_time), uncertainty = as.numeric(input$event_uncertainty))

    parameters$dropout_prior_distribution = ModuleEPriorDistribution(expected = log(2) / as.numeric(input$dropout_median_time), uncertainty = as.numeric(input$dropout_uncertainty))

    parameters$enrollment_prior_distribution = ModuleEPriorDistribution(expected = as.numeric(input$enrollment_median_time), uncertainty = as.numeric(input$enrollment_uncertainty))
  ...
  output$DownloadResults = downloadHandler(
    ...
    content = function(file) {
      ...
      # Run simulations  
      results = ModuleESimulation()

      doc = ModuleEReportDoc(results)
