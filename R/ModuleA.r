ModuleA = function(parameters) {
  ...
  simulations = ModuleAC(parameters)
...
ModuleAReportDoc = function(results) {

   #############################################################################

   # Error checks

   if (class(results) != "ModuleAResults") stop("The object was not created by the ModuleA function", call. = FALSE)
  ...
  if (endpoint_index %in% c(1, 2)) report_title = "Module A: Adaptive design with sample size re-estimation"

  if (endpoint_index %in% c(3)) report_title = "Module A: Adaptive design with event count re-estimation"
...
# End of ModuleAReportDoc


ModuleAReport = function(results, report_filename) {

  doc = ModuleAReportDoc(results)
