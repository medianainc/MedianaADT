ModuleC = function(parameters) {
  ...
  simulations = ModuleCC(parameters)
...
ModuleCReportDoc = function(results) {

   #############################################################################

   # Error checks

   if (class(results) != "ModuleCResults") stop("The object was not created by the ModuleC function", call. = FALSE)
  ...
  report_title = "Module C: Adaptive designs with data-driven population selection"
...
# End of ModuleCReportDoc


ModuleCReport = function(results, report_filename) {

  doc = ModuleCReportDoc(results)
