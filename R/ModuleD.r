ModuleD = function(parameters) {
  ...
  simulations = ModuleDC(parameters)
...
ModuleDReportDoc = function(results) {

   #############################################################################

   # Error checks

   if (class(results) != "ModuleDResults") stop("The object was not created by the ModuleD function.", call. = FALSE)
  ...
  report_title = "Module D: Optimal futility stopping rule"
...
# End of ModuleDReportDoc

ModuleDReport = function(results, report_filename) {

  doc = ModuleDReportDoc(results)
