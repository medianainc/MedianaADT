...
ModuleEPriorDistribution = function(expected,
                                    uncertainty) {
...
ModuleER = function(parameters) {
  ...
  for (k in 1:n_scenarios) {
    ...
    number_events[[k]] = ModuleEEventCount(current_parameters)$sim_results
...
ModuleEReportDoc = function(results) {

   #############################################################################

   # Error checks

   if (class(results) != "ModuleEResults") stop("The object was not created by the ModuleE function", call. = FALSE)
  ...
  report_title = "Module E: Event prediction in event-driven trials"
...
# End of ModuleEReportDoc


ModuleEReport = function(results, report_filename) {
