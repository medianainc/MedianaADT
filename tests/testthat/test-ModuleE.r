# Test the input of the ModuleE function

# Base case
baseCase = list(
  # Load the built-in data set with the patient enrollment, event and dropout 
  # information (ModuleEData)
  data_set = ModuleEData,
  ...
  event_prior_distribution = 
      ModuleEPriorDistribution(expected = log(2) / 15, uncertainty = 0.3),
  ...
  dropout_prior_distribution = 
      ModuleEPriorDistribution(expected = log(2) / 80, uncertainty = 0.3),
  ...
  enrollment_prior_distribution = 
      ModuleEPriorDistribution(expected = 35, uncertainty = 0.3),

  # Number of simulations
  nsims = 1000
)

context("ModuleE - Success runs")

test_that("Success run ModuleE", {
  ...
  expect_is(results, "ModuleEResults")
  ...
  ModuleEReport(results, tempfile(fileext = ".docx"))
})

context("ModuleE - Error checks")

test_that("Input parameters errors check ModuleE", {
  # Errors check
  expect_error(
    ModuleE(
      c("Not a list")
  ...
  brokenData = subset(ModuleEData, select=-c(enrollment))
  expect_error(
    ModuleE(
      list(
        # broken dataset
  ...
  # Drop valiable from dataset
  brokenData = subset(ModuleEData, select=-c(time))
  expect_error(
    ModuleE(
      list(
        # broken dataset
  ...
  # Drop valiable from dataset
  brokenData = subset(ModuleEData, select=-c(event))
  expect_error(
    ModuleE(
      list(
        # broken dataset
  ...
  # Drop valiable from dataset
  brokenData = subset(ModuleEData, select=-c(dropout))
  expect_error(
    ModuleE(
      list(
        # broken dataset
...
test_that("Input parameters errors check ModuleEReport", {
