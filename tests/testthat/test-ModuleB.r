# Test the input of the ModuleB function
...
context("ModuleB - Success runs")

test_that("Success run ModuleB with Normal case", {
  ...
  expect_is(results, "ModuleBResults")
  ...
  ModuleBReport(results, tempfile(fileext = ".docx"))
})

test_that("Success run ModuleB with Binary case", {
...
test_that("Success run ModuleB with Time-to-event case", {

  # Success run
  results = ModuleB(timeToEventCase)
  expect_is(results, "ModuleBResults")
  expect_type(  results$sim_results, "double")
  expect_length(results$sim_results, 13000)
...
test_that("Default values for dropout rate check ModuleB", {
  ...
  results = ModuleB(
    list(
      ...
      #dropout_rate = normalCase$dropout_rate,
      alpha = normalCase$alpha,
      nsims = 100 # normalCase$nsims
    )
  )
  expect_is(results, "ModuleBResults")
  expect_type(  results$sim_results, "double")
  expect_length(results$sim_results, 1300)
...
context("ModuleB - Error checks")

test_that("Input parameters errors check ModuleB", {
...
test_that("Input parameters errors check ModuleBReport", {
